...
class QualityReport:
    ...
    def check(self, dimension: Dimension, name: str, measured: float, threshold: float,
              higher_is_better: bool = True, detail: str = "") -> QualityCheck:
    ...
    def score(self) -> dict:
        ...
        result = {
            dim: round(sum(passes) / len(passes), 4)
            for dim, passes in by_dim.items()
        }
    ...
    def to_markdown(self, path: str) -> None:
        ...
        lines += ["", "## Checks", "", "| Dimension | Check | Measured | Threshold | Status | Detail |", "|---|---|---|---|---|---|"]
        for c in self.checks:
            status = "PASS" if c.passed else "**FAIL**"
            lines.append(f"| {c.dimension.value} | {c.name} | {c.measured} | {c.threshold} | {status} | {c.detail} |")
