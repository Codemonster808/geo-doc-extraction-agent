...
def retrieve_relevant_context(report_id: str, fallback_text: str) -> str:
    ...
    try:
        ...
        # utils/vectors.py docstring for the bug this fixes).
        ...
        indices = sorted(
            ...
            for r in top
            if r["metadata"]["chunk_index"] < len(sentences)
        )
...
def extract_with_confidence_gate(report_id: str, report_text: str) -> dict:
    ...
    """
    from orchestration.statemachine import gate_attempt
...
def _persist_extraction(report_id: str, record: dict) -> None:
    ...
    table src/transformation/resolve.py's cross-document entity resolution reads from."""
