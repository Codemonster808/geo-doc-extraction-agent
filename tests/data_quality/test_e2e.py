...
def gateway_process():
    ...
    # Forward the full environment (not just GIN_MODE) so the gateway
    # subprocess inherits AWS_ENDPOINT_URL/AWS_REGION/credentials from
    # env.sh — a bare {"GIN_MODE": "release"} silently drops those and
    # the gateway falls back to its hardcoded localhost:4566 default,
    # which is wrong whenever this repo's MiniStack runs on a different
    # port (e.g. several sibling repos' stacks running concurrently).
...
def test_full_pipeline_quality(gateway_process):
    ...
    doc_text = (
        f"E2E test report {run_id}: identical content should be rejected as a duplicate upload."
    )
    first = requests.post(
        f"{GATEWAY_URL}/upload", json={"report_id": f"E2E-{run_id}-a", "text": doc_text}
    )
    dup = requests.post(
        f"{GATEWAY_URL}/upload", json={"report_id": f"E2E-{run_id}-b", "text": doc_text}
    )
    malformed = requests.post(
        f"{GATEWAY_URL}/upload", json={"report_id": f"E2E-{run_id}-c", "text": "short"}
    )

    # --- extraction: schema validation domain checks (no LLM needed) ---
    from pydantic import ValidationError

    from models.extraction_agent import ExtractedRecord

    bad_coords_rejected = False
    try:
        ExtractedRecord(
            mineral="Gold", depth_m=100.0, lat=40.7, lon=-74.0, grade_g_t=1.0, hole_id="DH-1"
        )
    ...
    fail_result = extract_with_confidence_gate(
        f"e2e-fail-test-{run_id}", "Some report text with no real content."
    )
    ...
    resolve = subprocess.run(
        [sys.executable, "src/transformation/resolve.py"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        timeout=120,
    ...
    report.check(
        Dimension.VALIDITY,
        "gateway_accepts_new_content",
        measured=1.0 if first.status_code == 200 else 0.0,
        threshold=1.0,
        detail=f"first upload status={first.status_code}",
    )
    report.check(
        Dimension.VALIDITY,
        "gateway_rejects_duplicate_content",
        measured=1.0 if dup.status_code == 409 else 0.0,
        threshold=1.0,
        detail=f"duplicate upload status={dup.status_code}",
    )
    report.check(
        Dimension.VALIDITY,
        "gateway_rejects_malformed_upload",
        measured=1.0 if malformed.status_code == 400 else 0.0,
        threshold=1.0,
    ...
    report.check(
        ...
        "confidence_gate_fails_gracefully_not_silently",
        measured=1.0
        if fail_result["status"] == "failed" and fail_result["attempts"] == MAX_ITERATIONS
        else 0.0,
        threshold=1.0,
        detail=f"fail_result={fail_result['status']}, attempts={fail_result['attempts']}",
    )
    report.check(
    ...
    report.check(
        ...
        "resolve_job_under_sla",
        measured=round(resolve_seconds, 1),
        threshold=120.0,
        higher_is_better=False,
        detail="PySpark cross-doc resolution wall time",
    )
